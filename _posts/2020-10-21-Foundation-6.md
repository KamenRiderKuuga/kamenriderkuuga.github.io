...
`count-change`产生出一个树形的递归计算过程，其中的冗余计算与前面`fib`的第一种实现类似(时间复杂度较高)，另一方面，如果想设计出一个更好的算法，使其能算出同样结果，就不那么明显了。人们认识到，树形递归计算过程有可能及其低效，但常常很容易描述和理解，这就导致人们提出一个建议，希望能利用这两个最好的东西。人们希望能设计出一种"灵巧编译器"，使之能将一个树形递归的过程翻译为一个能计算出同样结果的更有效的过程。